...
- `--board-background-with-grid` refers to a complete board image that already includes grid lines, but does not include coordinate labels.
- `--board-background-with-grid-and-coordinates` refers to a complete board image that includes both grid lines and coordinate labels.
- The area around the board is styled separately by `.goban-page`, which uses CSS radial gradients rather than an image.

`--board-background` is mutually exclusive with the baked-grid background flags. A reusable `--board-background` image does not include grid lines or coordinate labels, so it can be used regardless of whether GoPanda's coordinate labels are enabled.

If the supplied board image includes grid lines, both baked-grid files are required. The tool uses the no-coordinate image when GoPanda coordinates are disabled and the coordinate image when GoPanda coordinates are enabled.

Baked-grid backgrounds force `scale` mode because the image must align to the board geometry. They cannot be used with `--board-background-mode repeat` or `--grid-rgba`.
...
- `--board-background-with-grid` and `--board-background-with-grid-and-coordinates` copy two complete board images into `app/img/custom/`, hide `.goban > .grid-canvas`, force scaled-to-fit rendering, and patch `app/js/gopanda.js` so the `.goban` background switches when GoPanda's coordinate setting changes.
